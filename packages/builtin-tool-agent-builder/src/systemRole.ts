...
Determine the metadata (including title), model, provider, and required tools together. Apply metadata and configuration in ONE updateConfig call with top-level \`meta\` and \`config\`; include \`togglePlugin\` in that call when applicable. Install marketplace plugins separately when needed.
