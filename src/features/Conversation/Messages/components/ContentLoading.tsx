...
const ContentLoading = memo<ContentLoadingProps>(({ id, startTime: startTimeOverride }) => {
  ...
  const llmRetryLabel = llmRetry
    ? t('opStatusTray.status.retrying', {
        attempt: llmRetry.attempt ?? '?',
        max: llmRetry.maxAttempts ?? '?',
      })
    : undefined;
  const loadingLabel = streamRetryLabel ?? llmRetryLabel ?? getOperationLabel();
  ...
  if (loadingLabel) {
    return (
      <Flexbox horizontal align={'center'} gap={4}>
        <span className={operationLabelClassName}>{loadingLabel}...</span>
        {showElapsedTime && (
