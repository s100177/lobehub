...
class ServerLLMRetryPolicy implements LLMRetryPolicy {
  ...
  async onRetry({ attempt, delayMs, error, maxAttempts }: LLMRetryInput) {
    ...
    await this.ctx.onLLMRetry?.({
      attempt: attempt + 1,
      delayMs,
      errorType: error.code ?? error.kind,
      kind: error.kind,
      maxAttempts,
    });
  }
  ...
  async recordRecovery() {
    if (!this.awaitingRecovery) return;
    this.awaitingRecovery = false;
    await this.ctx.onLLMRetryRecovered?.();
  }
...
export class ServerLLMTransport implements LLMTransport {
  ...
  private readonly serverRetryPolicy: ServerLLMRetryPolicy;
  ...
  constructor(
    ...
    this.serverRetryPolicy = new ServerLLMRetryPolicy(ctx);
    this.retryPolicy = this.serverRetryPolicy;
  }
  ...
  private async runAttemptWithRuntime(
    ...
    try {
      ...
      await this.serverRetryPolicy.recordRecovery();
