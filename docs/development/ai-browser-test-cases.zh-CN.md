...
- 即便有技能包，风险动作仍不得自动执行。
- 允许本次风险动作时，AI 只能交还给用户手动完成，不能自动越过风险门。
...
- 服务端必须记录当前人工干预版本已被 `inspect` 覆盖；仅伪造 `inspectedAfterIntervention:true` 不能恢复执行。
...
- “我手动处理” 必须调用 `interrupt` 写入服务端审计，状态进入 `paused_by_user_intervention`，后续继续仍需要重新 `inspect`。
- “取消任务” 必须调用 `cancelTask` / `/cancel-task` 写入服务端审计，状态进入 `cancelled`，不得再被当作可恢复暂停自动继续。
...
- 真实业务系统 URL 和技能包可用时，使用 `BROWSER_RELEASE_GATE_INCLUDE_BUSINESS_EVIDENCE=1 pnpm test:browser-release-gate` 把真实业务 evidence 纳入同一条门禁。
- L0 页面技能包静态校验通过。
- L1 本地可控测试全通过。
- L2 自动化脚本全通过。
- L3 真实网站冒烟无阻塞问题。
